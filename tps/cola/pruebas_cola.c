...
static void prueba_crear_destruir(void) {
    cola_t *c = cola_crear();
    print_test("la cola 1 fue creada", c);
    cola_destruir(c, NULL);

    cola_t *c_2 = cola_crear();
    print_test("la cola 2 fue creada", c_2);
    print_test("la cola 2 esta vacia", cola_esta_vacia(c_2));
    int* elem = malloc(sizeof(int));
    print_test("se creo el elemento de prueba", elem);
    bool encola = cola_encolar(c_2, elem);
    print_test("encola - cola 2", encola);
    int* check = cola_ver_primero(c_2);
    print_test("encola correctamente - cola 2", elem == check);
    cola_destruir(c_2, NULL);
    free(elem);

    cola_t *c_3 = cola_crear();
    print_test("la cola 3 fue creada", c_3);
    print_test("la cola 3 esta vacia", cola_esta_vacia(c_3));
    int* elem_3 = malloc(sizeof(int));
    print_test("se creo el elemento de prueba", elem_3);
    encola = cola_encolar(c_3, elem_3);
    print_test("encola - cola 3", encola);
    int* check_3 = cola_ver_primero(c_3);
    print_test("encola correctamente - cola 3", elem_3 == check_3);
    cola_destruir(c_3, free);
}
...
static void prueba_destruccion_free(void) {
    ...
    *t = TEST_ELEM;
    ...
    print_test("encolar da true", encola);
    cola_destruir(c, free);     // las otras 2 formas estan en crear_destruir
}
